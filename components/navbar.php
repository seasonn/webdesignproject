<nav class="navbar navbar-expand-lg bg-body-tertiary sticky-top flex-column py-1 px-3">
  <div class="container-fluid w-100 d-flex align-items-center">
    ...
    <?php 
		...
		$cart_rs = $conn->query($SQLstring);
		?>
    <div class="col-1 text-center text-lg-end"><button class="btn fs-4 position-relative"><i class="bi bi-cart"></i><span class="position-absolute top-50 start-50 translate-x badge rounded-pill bg-danger" style="font-size: 40%;"><?=($cart_rs) ? $cart_rs->rowCount():'';?></span></button></div>
  ...
  <div class="container-fluid">
    <div class="collapse navbar-collapse" id="navbarNavList">
      <div class="navbar-nav d-sm-flex w-100 justify-content-between flex-sm-row">
        <div class="d-md-flex col-sm-8 gap-3 mx-3">
          <?php multiList(); ?>
        </div>
        <hr class="d-sm-none my-2 col-sm-4">
        <div class="text-sm-end py-2 py-sm-0 mt-md-1">
